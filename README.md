...
flowchart TD
    ...
    Q --> S["网页获得自己的 device 身份，后续上传预绑定到 EXE owner"]
```
...
- 上传归属由 `Authorization` 里的 API Key/JWT 和服务端 `user_claims` 预绑定关系共同决定。
- SDK 会在设备模式上传时自动携带服务端返回的展示 `bind_code` 到上传 payload；服务端不会因为这个码把 master API Key 或正式账号身份发给 EXE。
...
3. 上传 payload 会自动携带该 `bind_code`；如果该设备已经预绑定到主账号，服务端会把资源归到主账号 owner 名下。
...
6. 如果网页还没有身份，网站会用 `?bind=` 创建自己的 device 身份并预绑定到 EXE owner；这条路径用于后续上传归属，不会继承 EXE 或主账号 API Key。
...
- 如果网页没有身份，调用 `/auth/device` 并带 `bind_code` 做预绑定；此时网页获得自己的 device 身份，后续上传会归属到该绑定码 owner。
- 管理已有资源走 claim；预绑定只建立后续上传归属关系，不会返回 EXE 或主账号 API Key。
...
| `bind_code` | 服务端 | 服务端用户字段 | 同一身份不变 | 建立资源认领或后续上传预绑定关系，不授予 API Key。 |
...
| 用例 | 覆盖点 |
|---|---|
| `test_q1_browser_guest_upload_then_exe_prebind_syncs_files` | 浏览器游客上传后，EXE 用浏览器绑定码预绑定；EXE 后续上传归到浏览器 owner，但 EXE 不继承浏览器 API Key。 |
...
| `test_q4_exe_first_upload_then_community_url_prebinds_browser_future_uploads` | EXE 先上传，浏览器无身份时通过 `/resources?bind=` 创建自己的 device 身份；后续上传归到 EXE owner。 |
...
不会。SDK 只把 `bind_code` 放入上传请求 payload，作为服务端可用的归属解析和审计元数据；当前网站源码并不把这个字段写进 `.char` / `.bg` 文件本体。最终归属仍由服务端结合 Bearer 身份、上传 key 和 `user_claims` 判断。

### 上传归属由 `uploader` 决定吗？

不是。`uploader` 只是页面展示字段。资源归属由服务端根据当前 Bearer 认证身份和 `user_claims` 预绑定关系决定。
