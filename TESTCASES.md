...
普通 EXE 不建议实现“管理我的资源”本地页面。用户自己的资源编辑、删除、改名和标签维护建议统一交给网站登录态完成；SDK 侧只负责上传、展示绑定码、打开社区绑定页。作者本体如果只是展示或下载公开资源，应使用全站公开列表 `/api/resources?offset=0&limit=100` 和资源对象里的 `url` 直链，不需要登录态。
